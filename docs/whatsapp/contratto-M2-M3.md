...
| 2026-08-03 | §4.1, riga `wa_campaign_contacts: status runtime...`: chiarito che M4 scrive una **seconda via** a `status=replied`, in aggiunta a quella che M3 già ha (guardia pre-invio, `wa_sender._esito_guardia_negativa`) | La riga diceva solo "M3 tutto il resto" senza nominare `replied` esplicitamente, scritta il 29/07 prima che M4 esistesse come cantiere — letta la prima volta come assegnazione esclusiva a M3, corretto dopo aver riletto `wa_sender.py:484` (la guardia scrive già `replied` da chat aperta). Stessa ridondanza intenzionale già documentata per `opted_out`. `replied_at_step` (riga sotto) resta solo di M4. Nessun altro punto di attrito trovato in §4.1: `dnc_reason`, `last_replied_at`, `wa_inbound_events` erano già esplicitamente di M4 | lead, pianificando M4 |
