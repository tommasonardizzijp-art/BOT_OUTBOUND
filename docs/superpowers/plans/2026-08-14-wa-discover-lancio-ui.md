...
> **Correzione 15/08, poche ore dopo il merge.** Il codice sotto (step 4) fa `puo_scansionare_lettura` rifiutare su `"ignota"` e `_esegui_scan` (step 5) attende e riprova 0/5/15s prima di arrendersi. Sul campo si è visto che era la scelta sbagliata: il selettore non trova MAI la voce Impostazioni su questo profilo (verificato di nuovo il 15/08, due sessioni distinte), quindi lo stato è sempre `"ignota"` e il gate rifiuta **ogni** scansione — da difetto silenzioso (gate che non ha mai funzionato, sopra) a discover completamente spento, che è peggio. Fail-closed su un segnale che non si sa leggere non è prudenza, è spegnere il sistema. Deciso: `puo_scansionare_lettura` su `"ignota"` ritorna `(True, ...)` e lo dichiara nel motivo; il ritentativo (0/5/15s) è tolto (un selettore sbagliato non migliora aspettando); `sync_stato="ignota"` resta registrato per la diagnosi (è quello che la UI del Task 9 mostra come "primo indiziato"); il motivo `sync_ignota` non è più un motivo di rifiuto ed è stato tolto da `MOTIVI_NON_GUASTO`. La vera rete di sicurezza è la misura di copertura (G7, `raccolta_parziale` sotto l'80%), non questo gate. La ricattura del selettore (step 1) resta da fare e a quel punto, se il gate torna a leggere davvero, si potrà tornare a farlo rifiutare su `"ignota"` — non prima.
