...
verso /reels/, fa scroll (una wheel decisa = reel successivo), e non deve mai
sollevare — un errore di navigazione ricade su un semplice sleep."""
import pytest
...
async def test_browse_reels_navigation_error_falls_back_no_raise(monkeypatch):
    ...
    # non deve sollevare: l'eccezione in navigazione ricade su asyncio.sleep(duration)
    await ig.browse_reels(0.05)
