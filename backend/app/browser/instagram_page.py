...
class InstagramPage:
    ...
    # ── Inbox DM scraping methods ──────────────────────────────────────────────

    async def open_inbox(self) -> None:
        """Naviga a /direct/inbox/ e dismetti eventuali modali.

        # VERIFY-LIVE: selettore dismiss e wait_until adeguati per l'inbox.
        """
        page = await self._get_page()
    ...
    async def scroll_inbox_threads(self) -> list[dict]:
        ...
        # VERIFY-LIVE: SELETTORI DA CONFERMARE SU SESSIONE IG REALE.
        # Selettore lista: 'div[role="list"] a[href^="/direct/t/"]'
        # Selettore username: 'span[dir="auto"]' all'interno di ogni link.
        # pk non esposto dal DOM: si ritorna None; risoluzione username->pk
        # via instagrapi user_id_from_username e' la strada da valutare live
        # (opzione a del brief). Alternativa: salvare pk da Fase Bio (opzione b).
        """
        page = await self._get_page()
    ...
    async def close_inbox_tabs(self) -> None:
        """Chiudi la pagina aperta per l'inbox (cleanup dopo scraping).

        # VERIFY-LIVE: verificare che la pagina sia sempre disponibile al cleanup.
        """
        try:
            ...
            logger.debug(f"close_inbox_tabs: {e}")
